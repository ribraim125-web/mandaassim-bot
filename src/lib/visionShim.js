/**
 * visionShim.js — substitui o cliente da Anthropic (Haiku) por GPT-5 mini via OpenRouter.
 *
 * Drop-in: expõe a MESMA interface `messages.create(params)` do SDK da Anthropic
 * (system como string ou array de blocos, content com blocos image/text) e devolve
 * a resposta no MESMO formato ({ content: [{ text }], usage: { input_tokens, ... } }).
 * Assim os módulos que usavam Haiku migram trocando só o require — prompts, parsing
 * e tracking ficam 100% intactos.
 *
...
// system da Anthropic pode ser string ou array de blocos { type:'text', text }
...
// Converte um bloco de content da Anthropic pro formato OpenAI
...
/**
 * Mesma assinatura de anthropic.messages.create(). Ignora params.model — o
 * modelo real é sempre VISION_MODEL (GPT-5 mini), tunável via .env.
 */
async function create(params) {
  ...
  // Formato de resposta espelha o SDK da Anthropic (o que os callers já leem)
  return {
